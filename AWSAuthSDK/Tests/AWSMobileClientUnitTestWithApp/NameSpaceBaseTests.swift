...
class NameSpaceBaseTests: XCTestCase {
    ...
    func checkInitializeAndSignedOut(_ configuration: [String: Any] ) {
        let initializeExpectation = expectation(description: "AWSMobileClient initialization")
        let mobileClient1 = AWSMobileClient(configuration: configuration)
        mobileClient1.initialize { (userState, error) in
            XCTAssertNil(error)
            initializeExpectation.fulfill()
        }
        wait(for: [initializeExpectation], timeout: 5)
        XCTAssertEqual(mobileClient1.currentUserState, .signedOut)
        XCTAssertEqual(mobileClient1.federationProvider, .none)
    }
    
    func checkInitializeAndUserPoolSignIn(_ configuration: [String: Any] ) {
        let initializeExpectation = expectation(description: "AWSMobileClient initialization")
        let mobileClient1 = AWSMobileClient(configuration: configuration)
        mobileClient1.initialize { (userState, error) in
            XCTAssertNil(error)
        ...
        wait(for: [initializeExpectation], timeout: 5)
        XCTAssertEqual(mobileClient1.currentUserState, .signedIn)
        XCTAssertEqual(mobileClient1.federationProvider, .userPools)
    }
